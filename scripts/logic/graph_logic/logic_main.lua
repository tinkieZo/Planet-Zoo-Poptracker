...
planet_zoo_location = {}
planet_zoo_location.__index = planet_zoo_location
...
---@type table<string, planet_zoo_new_return>
NAMED_LOCATIONS = {}
local stale = true
...
---@field name string
---@field side string?
---@field exits table<integer, {[1]:planet_zoo_new_return, [2]:fun(): accessibilityLevel}>
---@field keys integer
...
---@param name string
---@return planet_zoo_new_return
function planet_zoo_location.new(name)
	if name == nil then
		error("planet_zoo_location name cannot be nil")
	end
	if NAMED_LOCATIONS[name] then
		print("planet_zoo_location " .. name .. " already exists")
	end
    local self = setmetatable({}, planet_zoo_location)
	self.name = name
...
---@param exit string|planet_zoo_new_return planet_zoo_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function planet_zoo_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = planet_zoo_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|planet_zoo_new_return planet_zoo_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function planet_zoo_location:connect_two_ways(exit, rule)
...
---@param exit string|planet_zoo_new_return planet_zoo_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function planet_zoo_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|planet_zoo_new_return planet_zoo_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function planet_zoo_location:connect_two_ways_entrance(name, exit, rule)
...
function planet_zoo_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? fun(): accessibilityLevel|boolean
---@param rule2? fun(): accessibilityLevel|boolean
function planet_zoo_location:connect_two_ways_stuck(exit, rule1, rule2)
...
---@return accessibilityLevel
function planet_zoo_location:accessibility()
...
---@param keys integer
function planet_zoo_location:discover(accessibility, keys)
...
Entry_Point = planet_zoo_location.new("Entry_Point")
